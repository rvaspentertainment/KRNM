...
async def add_caption(client, message):
    ...
    await message.reply_text("__**✅ Yᴏᴜʀ Cᴀᴘᴛɪᴏɴ Iꜱ Sᴀᴠᴇᴅ Dᴜᴅᴇ**__")
   
@Client.on_message(filters.private & filters.command('del_caption'))
async def delete_caption(client, message):
    caption = await db.get_caption(message.from_user.id)  
    if not caption:
       return await message.reply_text("__**Sᴏʀʀʏ Dᴜᴅᴇ Yᴏᴜ Dᴏɴ'ᴛ Hᴀᴠᴇ Aɴʏ Cᴀᴘᴛɪᴏɴ**__")
    await db.set_caption(message.from_user.id, caption=None)
    await message.reply_text("__**❌️ Cᴀᴘᴛɪᴏɴ Sᴜᴄᴄᴇꜱꜱғᴜʟʟʏ Dᴇʟᴇᴛᴇᴅ Dᴜᴅᴇ**__")
...
async def addthumbs(client, message):
    ...
    await mkn.edit("✅️ __**Hᴇʏ ᴅᴜᴅᴇ ᴛʜᴜᴍʙɴᴀɪʟ ꜱᴀᴠᴇᴅ ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟʏ**__")
